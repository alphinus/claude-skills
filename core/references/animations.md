# {{BRAND_NAME}} Animations & Gamification Reference
...
@keyframes badgePulse {
  0%, 100% { filter: drop-shadow(0 0 6px rgba({{PRIMARY_RGB}},0.15)); }
  50% { filter: drop-shadow(0 0 14px rgba({{PRIMARY_RGB}},0.35)); }
}
...
class Particle {
  ...
  draw(ctx) {
    ...
    ctx.fillStyle = `rgba({{PRIMARY_RGB}}, ${this.life * 0.8})`;
    ctx.fill();
  }
}
  ...
  for (let i = 0; i < particles.length; i++) {
    for (let j = i + 1; j < particles.length; j++) {
      ...
      if (dist < maxDist) {
        ...
        ctx.strokeStyle = `rgba({{PRIMARY_RGB}},${0.15 * (1 - dist/maxDist)})`;
        ctx.lineWidth = 0.5;
        ctx.stroke();
      }
    }
...
function fireConfetti(canvas, ctx) {
  const colors = ['{{PRIMARY}}','{{PRIMARY_DARK}}','{{WARNING}}','#ed64a6','#9f7aea','#48bb78'];
  for (let i = 0; i < 150; i++) {
    particles.push({
...
.hover-glow:hover {
  box-shadow: 0 0 20px rgba({{PRIMARY_RGB}},0.15);
...
.profile-avatar {
  ...
  font-size: 1.8rem; font-weight: 800; color: {{BG}};
}
```
...
.activity-dot { width: 8px; height: 8px; border-radius: 50%; background: {{PRIMARY}}; }
...
.perk-icon.active { background: rgba({{PRIMARY_RGB}},0.12); }
```
...
.rm-circle.completed { background: rgba({{PRIMARY_RGB}},0.15); border-color: {{PRIMARY}}; color: {{PRIMARY}}; }
.rm-circle.current { background: {{PRIMARY}}; color: {{BG}}; box-shadow: 0 0 20px rgba({{PRIMARY_RGB}},0.4); }
.rm-circle.locked { background: rgba(255,255,255,0.02); border-color: rgba(255,255,255,0.1); color: {{MUTED}}; }
.rm-circle.milestone { width: 48px; height: 48px; }
.rm-line { width: 32px; height: 2px; flex-shrink: 0; }
.rm-line.done { background: {{PRIMARY}}; }
.rm-line.pending { background: rgba(255,255,255,0.08); }
.rm-label { font-size: 0.7rem; color: {{MUTED}}; margin-top: 8px; }
```
...
const levels = [
  ...
  { n: 20, label: "{{BRAND_NAME}}-Meister", milestone: true }
];
```
