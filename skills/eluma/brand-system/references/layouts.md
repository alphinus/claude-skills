# Eluma Layouts & Navigation Reference
...
```html
<aside class="sidebar">
  <div class="sidebar-logo">Eluma</div>
...
.nav-item {
  ...
  padding: 11px 20px;
  color: #718096; font-size: 13px; font-weight: 400;
...
.chart-bar {
  ...
  min-height: 8px;
  background: linear-gradient(to top, rgba(79,209,197,0.2), rgba(79,209,197,0.6));
  transition: all 0.3s;
}
.chart-bar:hover {
  background: linear-gradient(to top, rgba(79,209,197,0.3), #4fd1c5);
}
```
...
.brand-panel {
  ...
  background: linear-gradient(135deg, rgba(30,58,95,0.3), #0f172a);
}
.form-panel {
  width: 480px;
  display: flex; flex-direction: column;
...
.topbar-logo { font-size: 13px; letter-spacing: 4px; color: #4fd1c5; font-weight: 300; }
```
...
// Persist preference
localStorage.setItem('eluma-theme', toggle.checked ? 'light' : 'dark');
```
...
.panel-tab {
  ...
  border: none; color: #718096; font-family: inherit;
}
.panel-tab.active {
  background: rgba(79,209,197,0.08);
  color: #4fd1c5;
...
.cmd-input {
  ...
  font-size: 15px; color: #f8fafc; font-family: inherit;
}
```
...
.step-dot {
  ...
  color: #718096; transition: all 0.4s; flex-shrink: 0;
}
.step-dot.active {
  background: rgba(79,209,197,0.1);
  border-color: #4fd1c5; color: #4fd1c5;
  box-shadow: 0 0 16px rgba(79,209,197,0.15);
}
.step-dot.done {
  background: #4fd1c5;
  border-color: #4fd1c5;
...
.step-line-fill { height: 100%; background: #4fd1c5; width: 0; transition: width 0.6s ease; }
```
...
.divider {
  ...
  margin: 24px 0; color: #718096; font-size: 12px;
}
...
.event-row:hover { background: rgba(79,209,197,0.04); }
...
.activity-dot {
  width: 8px; height: 8px; border-radius: 50%;
  background: #4fd1c5; margin-top: 6px; flex-shrink: 0;
}
.activity-text { font-size: 13px; color: #718096; line-height: 1.5; }
